...
const EMPTY_SESSION_LABEL = "بدون جلسة";
const startCopy = {
...
function setupEvents() {
  ...
  hud.shooterModeButton.addEventListener("click", () => setGameMode(GAME_MODES.SHOOTER));
  hud.racingModeButton.addEventListener("click", () => setGameMode(GAME_MODES.RACING));
  hud.resumeButton.addEventListener("click", resumeGame);
  hud.restartButton.addEventListener("click", () => restartGame(true));
  hud.connectButton.addEventListener("click", () => connectNetwork());
  hud.createMatchButton.addEventListener("click", () => createNetworkMatch());
  hud.joinMatchButton.addEventListener("click", () => joinNetworkMatch(getMatchIdInput()));
  hud.settingsConnectButton.addEventListener("click", () => connectNetwork());
  hud.settingsCreateMatchButton.addEventListener("click", () => createNetworkMatch());
  hud.settingsJoinMatchButton.addEventListener("click", () => joinNetworkMatch(getMatchIdInput()));
  hud.settingsDisconnectButton.addEventListener("click", () => disconnectNetwork());
  ...
  hud.sensitivity.addEventListener("input", () => {
...
function connectNetwork(afterConnect = null) {
  ...
  try {
    ...
    socket.addEventListener("open", () => {
      ...
      localStorage.setItem("neon_server_url", serverUrl);
      localStorage.setItem("neon_player_name", playerName);
      setNetworkStatus("متصل، بانتظار lobby");
    });
    ...
    socket.addEventListener("message", (event) => {
      try {
        ...
        setNetworkStatus("رسالة غير صالحة من السيرفر");
      }
    });
    ...
    socket.addEventListener("error", () => {
      setNetworkStatus("خطأ اتصال WebSocket");
    });
  } catch {
    network.connecting = false;
    setNetworkStatus("تعذر فتح WebSocket");
  }
}
...
function joinNetworkMatch(matchId) {
  ...
  if (!cleanMatchId) {
    setNetworkStatus("اكتب Match ID أو أنشئ مباراة جديدة");
    return;
  }
...
function handleNetworkMessage(message) {
  ...
  if (message.type === "connected") {
    ...
    setNetworkStatus(`متصل: ${localSnapshot.id}`);
  ...
  if (message.type === "lobby_matches" && !network.matchId) {
    ...
    setNetworkStatus(network.connected ? `متصل، مباريات متاحة: ${count}` : "غير متصل");
    return;
  }

  if (message.type === "player_joined") {
    upsertRemotePlayer(message.player);
    setNetworkStatus(`لاعب انضم: ${message.player?.name ?? "Player"}`);
    return;
  }
...
function syncMatchState(match) {
  ...
  setNetworkStatus(`داخل مباراة: ${match.id} | لاعبين: ${match.players?.length ?? 1}/${match.max_players ?? "?"}`);
...
function hydrateNetworkForm() {
  const storedServer = localStorage.getItem("neon_server_url");
  const storedName = localStorage.getItem("neon_player_name");
  setServerUrlInput(storedServer || hud.serverUrlInput?.value || DEFAULT_SERVER_URL);
